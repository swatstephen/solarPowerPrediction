...
def goPrd():
    ...
    #bins = [5,7.5,8.5,9.5,10.5,11.5,12.5,13.5,15.5,16,18]
    ...
    #出錯時檢查資料來源之氣象種類及時段，就是印出：showWea, showTim這兩個。看是否有或有少
    ...
    print(df)
    df.to_csv('預測記錄.csv',encoding='utf-8-sig',mode='a', index=False, header=False)
    df = df.to_string(index=False)
    linenotify(df,wea,mx,mi)
