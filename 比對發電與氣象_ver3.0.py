...
x = df[['外氣','wea1','wea2','wea3','wea4','T1','T2','T3','T4','T5','T6','T7','T8','T9']]
...
#y = w1*外氣+ w2*wea1....+b
w = np.array([x for x in range(1,15)])
b = 1
...
w_init = np.array([x for x in range(1,15)])
b_init = 0
...
datas = [['多雲',9,27,bins,'09:00'],['多雲',12,31,bins,'12:00'],['多雲',15,31,bins,'15:00']]
...
for d in datas:
    ns = predict(d[0],d[1],d[2],d[3])
    ns = np.around(ns, decimals=1)
    print(f'{d[4]}預測小時總發電{ns[0]}度','\n')
