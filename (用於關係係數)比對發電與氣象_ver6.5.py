...
#只選要顯示的欄位
df =  df[['時間','總發電量','氣象','外氣','濕度','風速']]
df2 =  df[['時間','總發電量','氣象','外氣','濕度','風速']]
...
df = df.drop(['氣象','時間','時段','wea5','T10'], axis = 1)

#print(df)
#print(df.corr())


x = df[['外氣','wea1','wea2','wea3','wea4','T1','T2','T3','T4','T5','T6','T7','T8','T9','濕度','風速']]
...
#y = w1*外氣+ w2*wea1....+b
w = np.array([x for x in range(1,17)])
b = 1
...
w_init = np.array([x for x in range(1,17)])
b_init = 0
...
df2['時段'] = df2['時間'].dt.hour
df2['氣象2'], unique_value = pd.factorize(df2['氣象'])
df2['時段'] = df2['時段'].astype(int)
print(df2)
print(df2['時段'])

cor1 = df2['總發電量'].corr(df2['氣象2'])
cor2 = df2['總發電量'].corr(df2['外氣'])
cor3 = df2['總發電量'].corr(df2['濕度'])
cor4 = df2['總發電量'].corr(df2['風速'])
cor5 = df2['總發電量'].corr(df2['時段'])
cor6 = df2['濕度'].corr(df2['外氣'])

print(f'總發電量和氣象的關係係數{cor1:.1f}')
print(f'總發電量和外氣的關係係數{cor2:.1f}')
print(f'總發電量和濕度的關係係數{cor3:.1f}')
print(f'總發電量和風速的關係係數{cor4:.1f}')
print(f'總發電量和時段的關係係數{cor5:.1f}')
print(f'濕度和外氣的關係係數{cor6:.1f}')
